...
export class AccountsController {
  ...
  list(@Request() req: any) {
    return this.listAccounts.execute(req.user.sub);
  }

  @Post()
  create(@Request() req: any, @Body() dto: CreateAccountDto) {
    return this.createAccount.execute(req.user.sub, dto);
  }

  @Patch(':id')
  update(@Request() req: any, @Param('id') id: string, @Body() dto: UpdateAccountDto) {
    return this.updateAccount.execute(req.user.sub, id, dto);
  }

  @Delete(':id')
  @HttpCode(HttpStatus.NO_CONTENT)
  remove(@Request() req: any, @Param('id') id: string) {
    return this.deleteAccount.execute(req.user.sub, id);
  }

  @Post(':id/holdings')
  addHolding(@Request() req: any, @Param('id') accountId: string, @Body() dto: CreateHoldingDto) {
    return this.createHolding.execute(req.user.sub, accountId, dto);
  }

  @Delete(':id/holdings/:holdingId')
  @HttpCode(HttpStatus.NO_CONTENT)
  removeHolding(@Request() req: any, @Param('id') accountId: string, @Param('holdingId') holdingId: string) {
    return this.deleteHolding.execute(req.user.sub, accountId, holdingId);
  }
}
